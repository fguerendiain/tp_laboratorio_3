...
$app->get('/crear_token/', function (Request $request, Response $response) {    
    $datos = array('nombre'=>'Juan','apellido'=>'Perez','edad'=>33);
    $token = autentificadorJwt::CrearToken($datos);
    $newResponse = $response->withJson($token, 200);  
    return $newResponse;
});

$app->get('[/verificar_token/]', function (Request $request, Response $response) {    
    ...
    if($isValid)
    {
        echo "Tooo PIola VErif";
    }
    return $response;

});

$app->get('[/error_token/]', function (Request $request, Response $response) {    
    $datos = "token";
    $token = autentificadorJwt::CrearToken($datos);
    autentificadorJwt::VerificarToken($token);
    $isValid = false;
